"""
AI Employee Vault — Gold Tier Main Entry Point.
...
Gold Tier adds: Odoo accounting, social media (FB/IG/Twitter),
Ralph Wiggum loop, error recovery, and 23 total skills.
...
# Initialize skill registry with all Gold tier skills (23 total)
...
# Gold tier
registry.register(OdooAccountingSkill)
registry.register(FacebookPostSkill)
registry.register(InstagramPostSkill)
registry.register(TwitterPostSkill)
registry.register(SocialMediaSummarySkill)
registry.register(WeeklyBusinessAuditSkill)
registry.register(ErrorRecoverySkill)
...
class ApprovalHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        elif "facebook" in content or "facebook" in src.name.lower():
            registry.run("facebook_post", src)
        elif "instagram" in content or "instagram" in src.name.lower():
            registry.run("instagram_post", src)
        elif "twitter" in content or "tweet" in src.name.lower():
            registry.run("twitter_post", src)
...
def main():
    ...
    log_entry("Gold Tier Watcher started. Monitoring Inbox and Approved folders.")
