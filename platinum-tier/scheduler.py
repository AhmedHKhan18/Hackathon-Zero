"""
Scheduler — Task scheduling for the AI Employee (Gold Tier).
...
- Error recovery check (every 15 minutes)
...
class AIEmployeeScheduler:
    ...
    def __init__(self, vault_path: str):
        ...
        self.registry.register(WeeklyBusinessAuditSkill)
        self.registry.register(ErrorRecoverySkill)
    ...
    def weekly_business_audit(self):
        ...
        print(f"[{datetime.now().strftime('%H:%M')}] Generating weekly business audit...")
        self.registry.run("weekly_business_audit")
        self.registry.run("update_dashboard")

    def error_recovery_check(self):
        """Check for and recover from errors."""
        result = self.registry.run("error_recovery")
        errors = result.get("errors_found", 0)
        if errors > 0:
            print(f"[{datetime.now().strftime('%H:%M')}] Error recovery: {errors} errors found")
    ...
    def setup_default_schedule(self):
        ...
        # Weekly business audit every Sunday (604800 seconds = 7 days)
        self.add_recurring("Weekly Audit", 604800, self.weekly_business_audit)

        # Error recovery check every 15 minutes
        self.add_recurring("Error Recovery", 900, self.error_recovery_check)
