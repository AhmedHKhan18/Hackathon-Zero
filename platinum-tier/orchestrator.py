    ...
    def process_needs_action(self):
        ...
        for f in self.needs_action.iterdir():
            ...
            try:
                ...
                # Detect action type from filename and content
                is_email = (
                    "type: email" in content
                    or f.name.upper().startswith("EMAIL_")
                )
                is_linkedin = (
                    "linkedin" in content
                    or "linkedin" in f.name.lower()
                )
                is_facebook = "facebook" in content or "facebook" in f.name.lower()
                is_instagram = "instagram" in content or "instagram" in f.name.lower()
                is_twitter = (
                    "twitter" in content or "tweet" in content
                    or "twitter" in f.name.lower()
                )
                ...
                if needs_approval:
                    ...
                    # Auto-execute based on detected action type
                    if is_email and f.exists():
                        try:
                            self.retry_handler.execute_with_retry(
                                self.registry.run, "gmail_send", f, service_name="gmail"
                            )
                            self.log_entry(f"Auto-replied to email: {f.name}")
                        except Exception as e:
                            self.log_entry(f"Email reply failed for {f.name}: {e}")

                    if is_linkedin and f.exists():
                        try:
                            self.retry_handler.execute_with_retry(
                                self.registry.run, "linkedin_auto_post", f, service_name="linkedin"
                            )
                            self.log_entry(f"Auto-posted to LinkedIn: {f.name}")
                        except Exception as e:
                            self.log_entry(f"LinkedIn post failed for {f.name}: {e}")

                    if is_facebook and f.exists():
                        try:
                            self.retry_handler.execute_with_retry(
                                self.registry.run, "facebook_post", f, service_name="facebook"
                            )
                            self.log_entry(f"Auto-posted to Facebook: {f.name}")
                        except Exception as e:
                            self.log_entry(f"Facebook post failed for {f.name}: {e}")

                    if is_instagram and f.exists():
                        try:
                            self.retry_handler.execute_with_retry(
                                self.registry.run, "instagram_post", f, service_name="instagram"
                            )
                            self.log_entry(f"Auto-posted to Instagram: {f.name}")
                        except Exception as e:
                            self.log_entry(f"Instagram post failed for {f.name}: {e}")

                    if is_twitter and f.exists():
                        try:
                            self.retry_handler.execute_with_retry(
                                self.registry.run, "twitter_post", f, service_name="twitter"
                            )
                            self.log_entry(f"Auto-posted to Twitter: {f.name}")
                        except Exception as e:
                            self.log_entry(f"Twitter post failed for {f.name}: {e}")
